...
; С define можем да дефинираме и функции:
; (define
;   (<name> <arg1> <arg2> ..)
;   <body>)

(define (square x) (* x x))
(define (divide-by-zero x) (/ x 0))

; В този случай тялото не се оценява стриктно,
; защото define също е специална форма.

; Това не е съвпадение. Горния синтаксис за дефиниране на функция
; е синтактична захар за свързване на символ с ламбда функция.
(define divide-by-zero-lambda (lambda (x) (/ x 0)))
...
; Също така работят над много аргументи
; и могат да се използват за не-булеви стойности.
(define get-first-number (or #f #f 2 #f 3)) ; 2
...
; Използвайте succ и pred. Не може да използвате +
(define (sum n m) 'undefined)

; 7. намира произведението на 2 естествени числа.
; Използвайте sum/succ и pred. Не може да използвате *
