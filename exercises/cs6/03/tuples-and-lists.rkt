...
; 11. Функция foldl, която "сгъва" списък от ляво надясно.
; За дадени списък (x1 x2 x3 ... xn), операция * и начална стойност nv,
; пресмята израза (...(((nv * x1) * x2) * x3) ... * xn).
;
; Реализирайте чрез опашкова рекурсия.
;
; Пример: (foldl* - 0 '(1 2 3 4)) -> -10
(define (foldl* op nv lst) 'undefined)

; 12. Функция foldr, която "сгъва" списък от дясно наляво.
; За дадени списък (x1 x2 x3 ... xn), операция * и начална стойност nv,
; пресмята израза (x1 * (x2 * (x3 * (... (xn * nv) ...)))).
;
; Реализирайте чрез примитивна рекурсия рекурсия.
;
; Пример: (foldr* - 0 '(1 2 3 4)) -> -2
(define (foldr* op nv lst) 'undefined)
